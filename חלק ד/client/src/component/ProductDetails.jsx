...
const ProductDetails = ({ product, setCount, setSum }) => {
    // הערך שמוצג בinput של כמות מינימלית שמאותחל במינימום ומשתנה בעת שינוי
    const [value, setValue] = useState(product.minCount);
    ...
    const handleChange = (e) => {
        ...
        if (isNaN(newValue) || newValue < product.minCount) {
            setValue(product.minCount);
        } else {
    ...
    return (
        <Card sx={{ maxWidth: 345, padding: 3, borderRadius: 2 }}>
            <CardContent>
                {/* אם אתה מנהל רק אז יהיה אפשרות לרכוש מוצר זה  */}
                ...
                <Typography variant="body1" color="text.secondary" sx={{ mb: 2 }}>
                    כמות מינימלית: {product.minCount}
                </Typography>
